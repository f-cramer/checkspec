package checkspec.eclipse.launching.ui.shortcuts;
...
import checkspec.eclipse.launching.classpath.ProjectClassPathEntry;
import checkspec.eclipse.launching.classpath.SourceClassPathEntry;
import checkspec.eclipse.launching.ui.CheckSpecUIPlugin;

public class CheckSpecLauncherShortcut implements ILaunchShortcut2 {

	private static final String PLUGIN_ID = "checkspec.eclipse.launching.ui";
	...
	private ILaunchConfigurationWorkingCopy createLaunchConfiguration(IJavaElement element) throws CoreException {
		...
		wc.setAttribute(Constants.ATTR_SPEC_TYPE_NAME, mainTypeQualifiedName);
		wc.setAttribute(Constants.ATTR_IMPL_PATH, implementationPath.toString());
	...
	private ClassPath getImplementationPath(IJavaProject project) {
		...
		try {
			...
			classPath.add(new SourceClassPathEntry(project.getOutputLocation()));
	...
	private IType[] findTypesToLaunch(ICompilationUnit cu) throws CoreException {
		try {
			...
			throw new CoreException(new Status(Status.ERROR, PLUGIN_ID, e.getMessage()));
		}
	}
	...
	protected String[] getAttributeNamesToCompare() {
		return new String[] { Constants.ATTR_SPEC_TYPE_NAME, IJavaLaunchConfigurationConstants.ATTR_PROJECT_NAME, };
	}

	public Shell getShell() {
		return CheckSpecUIPlugin.getActiveWorkbenchShell();
	}
